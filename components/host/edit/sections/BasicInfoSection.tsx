...
import { STAY_CATEGORIES } from '@/components/create-listing/types';
    ...
    <View style={s.section}>
      ...
      <View style={s.chipGrid}>
        {STAY_CATEGORIES.map(c => {
          ...
          return <TouchableOpacity key={c.value} style={[s.chip, { borderColor: sel ? tint : border, backgroundColor: sel ? `${tint}08` : 'transparent' }]} onPress={() => toggleCat(c.value)}><Ionicons name={c.icon as any} size={16} color={sel ? tint : text} /><Text style={[s.chipLabel, { color: sel ? tint : text }]}>{c.label}</Text></TouchableOpacity>;
        })}
      </View>
